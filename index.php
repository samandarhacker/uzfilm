...
$reklamauz = "<i>🔥Reklama: @freexost_robot - Tezkor Xosting</i>";
$reklamaru = "<i>🔥Реклама: @freexost_robot - Надёжный хостинг</i>";
...
if ($blok_info and $join_info and ($mp3) and $type == "private" and $text != "◀️Ortga") {
  ...
  if ($mp3) {
    ...
    $row = mysqli_fetch_assoc($result);
    $result2 = mysqli_query($connect, "SELECT * FROM `vdata` WHERE `size` = '$size'");
...
if (false) {
  ...
  $row7 = mysqli_fetch_assoc($result7);
  $lang = $row7['lang'];
  $res = mysqli_query($connect, "SELECT * FROM `vdata`ORDER BY top DESC LIMIT 10");
  while ($a = mysqli_fetch_assoc($res)) {
    $vid = $a['doc_id'];
    $f = $a['name'];
